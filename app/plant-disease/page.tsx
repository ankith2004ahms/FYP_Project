...
import { postWithClerkAuth, fetchWithClerkAuth } from "@/utils/clerk-fetch";
...
export default function PlantDiseasePage() {
  ...
  const saveDiseaseDetection = async () => {
    if (!isSignedIn || !userId || !diseaseData || !selectedImage) {
    ...
    try {
      ...
      const uploadResponse = await fetchWithClerkAuth('/api/upload', {
        method: 'POST',
        body: imageFormData,
      });
      ...
      // Then save the disease detection
      const detectionData = {
        imageUrl,
      ...
      const response = await postWithClerkAuth('/api/disease-detection', detectionData);
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to save disease detection');
      }
          ...
          {diseaseData && (
            <div className="bg-white rounded-lg shadow-lg p-8 mb-8 border border-gray-200 space-y-6">
              <div className="border-b pb-4 mb-4">
                ...
                {saveSuccess && (
                  <div className="mt-2 p-2 bg-green-50 border border-green-200 text-green-700 rounded-lg text-sm">
                    ✓ Saved to your history successfully!
                  </div>
                )}
              </div>
